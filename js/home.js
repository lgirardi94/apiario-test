...
function renderHome() {
  ...
  // Arnie cards
  const tipoEmoji2 = { covata:'C', covata_opercolata:'CO', scorte:'S', cov_scorte:'C+S', foglio_cereo:'FC', vuoto:'○', '':'?' };
  const colori = { covata:'#8B5E2A', covata_opercolata:'#C8860A', scorte:'#F5C842', cov_scorte:'#6BA05A', foglio_cereo:'#e3f2fd', vuoto:'#eee', '':'#eee' };
    ...
    : arnie.map(a => {
        ...
        const mappaHtml = isp?.ispezione?.mappa?.length ? `
          <div style="display:flex;gap:2px;margin-top:6px;flex-wrap:wrap">
            ${isp.ispezione.mappa.map((tipo,i) => `<div title="T${i+1}" style="width:22px;height:32px;border-radius:2px;border:1px solid rgba(0,0,0,0.12);background:${colori[tipo]||'#eee'};display:flex;align-items:center;justify-content:center;font-size:0.6rem;font-weight:700;color:${tipo==='scorte'?'#2A1A05':'white'}">${tipoEmoji2[tipo]||'?'}</div>`).join('')}
          </div>` : '';
        return `
